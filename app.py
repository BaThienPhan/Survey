...
# QR Code trỏ đến ứng dụng (thay đường dẫn theo streamlit share)
url = "https://surveyssss.streamlit.app/"  # Đổi đường dẫn này khi deploy
qr = qrcode.make(url)
buf = BytesIO()
qr.save(buf)
buf.seek(0)
st.image(Image.open(buf), caption="📱 Quét mã QR để truy cập khảo sát", width=200)

st.divider()

# Ngăn người dùng gửi nhiều lần (chỉ trong phiên session)
...
st.divider()
st.subheader("📊 Kết quả khảo sát (Cập nhật mỗi 5 giây)")
